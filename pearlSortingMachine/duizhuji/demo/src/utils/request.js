...
axios.interceptors.request.use(
  (config) => {
    if (localStorage.marbles_heardTokenStarter) {
      //判断token是否存在
      config.headers["Authorization"] =
        "Bearer " + localStorage.getItem("marbles_heardTokenStarter"); //将token设置成请求头
    }
    return config;
  },
  (e) => Promise.reject(e)
    ...
    if (error && error.response) {
      switch (
        ...
        case 401:
          ...
          localStorage.removeItem("marbles_heardTokenStarter");
          // delCookie('username');
          router.push({
            path: "/login",
          });
