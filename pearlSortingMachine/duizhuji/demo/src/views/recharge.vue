      ...
      <div class="col">
        ...
        >
          ...
          <van-col class="coins-bind" span="12">
            <p v-if="isCoin === false">得{{ item.coins }}元</p>
            <p v-else>{{ item.coins }}币</p>
            <p>赠送{{ item.coinsBind }}币</p>
          </van-col>
        </van-row>
      </div>
      <div class="combo" v-if="rechargeConfigList.length <= 0">
        暂无充值套餐
      </div>
    </div>
  </div>
...
const onshowShop = async (item) => {
  ...
  await v3PlayerPlaceOrderAPI({
    ...
    frontUrl: "https://www.huanxizn.com/newmarbles/#/home/index", //支付完成后，前端跳转地址
  })
    .then((res) => {
      showLoading.value = false;
