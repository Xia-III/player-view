...
const getOpenId = async (code) => {
    ...
    await v3AuthPlayerAuthAPI({
        ...
        if (res.data.code == 200) {
            ...
            setStorageExpire('clawcrane_tokenStarter', res.data.data, 1440);
            const host = window.location.host
            window.location.href = 'https://' + host + '/newclawcrane/#/toLogin'
...
onMounted(() => {
    ...
    if (ua.match(/MicroMessenger/i) == 'micromessenger') {
        if (getStorageExpire('clawcrane_tokenStarter')) {
            wxGetCode()
        }
    }
