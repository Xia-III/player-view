<template>
        ...
        <div class="bottommessage">
          <div class="box1" ref="box1Ref">
            <div class="box2">
              ...
              <van-button
                class="btn"
                v-for="(item, index) in sortedDeviceTypebtns"
                :key="item.id"
                :plain="item.type !== 'danger'"
                :type="machineId == item.id ? 'danger' : 'default'"
                @click="choiceDeviceType(index, item)"
              >
                <p style="font-size: 16px; font-weight: 700; padding: 3px">
                  {{ item.name }}
                </p>
                {{ item.id }}
              </van-button>
          ...
          >
            ...
            <transition name="fade-scale">
              <div
                ...
                @click.stop="handleGuidanceClick"
              >
                <!-- 精确圆形的背景蒙层，中心对齐 -->
  ...
  >
    ...
    <div class="coinsContent">
      <div
        class="coinPlan"
        v-for="(item, index) of rechargeConfigList"
        :key="index"
        v-if="rechargeConfigList.length > 0"
      >
...
onMounted(() => {
  const isFirstVisit = localStorage.getItem("is_first_visit");
...
const getRecharge = async () => {
  ...
  await v3PlayerRechargeConfigListAPI()
    .then((res) => {
      ...
      if (res.data.code == 200) {
        ...
        if (res.data.data) {
          for (let i = 0; i < res.data.data.length; i++) {
            ...
            res.data.data[i].coinsBind = Math.floor(
              res.data.data[i].coinsBind / 100,
            );
          }
...
const sortedDeviceTypebtns = computed(() => {
  if (!deviceTypebtns.value || deviceTypebtns.value.length === 0) return [];
  const btns = [...deviceTypebtns.value];
  const highlightedIndex = btns.findIndex((item) => item.id == machineId.value);
  if (highlightedIndex > 0) {
    const [highlightedItem] = btns.splice(highlightedIndex, 1);
    btns.unshift(highlightedItem);
  }
  return btns;
});
...
const choiceDeviceType = (index, item) => {
  ...
  remarkName.value = item.name;
  machineId.value = item.id;

  if (box1Ref.value) {
    box1Ref.value.scrollLeft = 0;
    console.log(`[Test] 切换至机器: ${item.id}, box1 已归位到最前端`);
  }
};
...
const toggleImage = (item) => {
  ...
  if (
    (coinsBind.value / coinFee.value >= count.value && coinsBind.value > 0) ||
    (coins.value / coinFee.value >= count.value && coins.value > 0)
  ) {
    //默认投币
...
.main {
  ...
  .circle-backdrop {
    ...
    box-shadow: 0 0 10px rgba(0, 0, 0, 0.2); /* 增加边缘过渡柔和感 */
  }

  .finger-guide {
    position: absolute;
    /* 调整位置，让手指指尖准确对准圆形中心，并与左上角形成视觉引导线 */
    top: 40%;
    left: 40%;
    width: 120px; /* 放大尺寸 (原 60px 的两倍) */
    height: 120px;
    animation: pulse 1s infinite ease-in-out; /* 调整动画时长 */
    transform-origin: 10% 10%; /* 以指尖为中心进行缩放 */
  }
  ...
  .fade-scale-leave-active {
    transition: all 0.3s cubic-bezier(1, 0.5, 0.8, 1); /* 300ms 内消失延迟 */
  }
