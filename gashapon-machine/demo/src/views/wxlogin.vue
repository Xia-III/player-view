...
const getOpenId = async (code) => {
    ...
    await v3AuthPlayerAuthAPI({
        ...
        if (res.data.code == 200) {
            ...
            localStorage.setItem('clawcrane_heardTokenStarter', res.data.data);
            // 设置登录的过期时间24小时即1440分钟 单位分钟
            setStorageExpire('gashaponMachine_tokenStarter', res.data.data, 1440);
            // router.push({
            //     path: '/toLogin'
            // })
            const host = window.location.host
            window.location.href = 'https://' + host + '/newGashapon/#/toLogin'
...
onMounted(() => {
    ...
    if (ua.match(/MicroMessenger/i) == 'micromessenger') {
        if (getStorageExpire('gashaponMachine_tokenStarter')) {
            wxGetCode()
        }
    }
