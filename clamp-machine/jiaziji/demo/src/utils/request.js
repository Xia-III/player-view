...
axios.interceptors.request.use(
  (config) => {
    if (localStorage.clamp_heardTokenStarter) {
      //判断token是否存在
      config.headers["Authorization"] =
        "Bearer " + localStorage.getItem("clamp_heardTokenStarter"); //将token设置成请求头
    }
    return config;
  },
  (e) => Promise.reject(e)
    ...
    if (error && error.response) {
      switch (
        ...
        case 401:
          ...
          localStorage.removeItem("clamp_heardTokenStarter");
          // delCookie('username');
          router.push({
            path: "/login",
          });
