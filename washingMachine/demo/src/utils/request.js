...
axios.interceptors.request.use(config => {
    if (localStorage.coiner_heardTokenStarter) { //判断token是否存在
        config.headers['Authorization'] = 'Bearer ' + localStorage.getItem('coiner_heardTokenStarter');  //将token设置成请求头
    }
    return config
}, e => Promise.reject(e))
    ...
    if (error && error.response) {
        switch (error.response.status) {  //处理状态码
            ...
            case 401:
                ...
                localStorage.removeItem('coiner_heardTokenStarter')
                // delCookie('username');
                router.push({
                    path: "/login",
                })
