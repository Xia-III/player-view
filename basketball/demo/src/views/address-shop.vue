<template>
  ...
  <div class="addressShop">
    <div class="item" v-for="(item, index) in addressIdList" :key="index">
      ...
      <van-button
        type="success"
        v-if="item.addressId != addressId"
        @click="activeMachine(item)"
        >切换当前场地</van-button
      >
      <van-button type="default" v-if="item.addressId == addressId"
        >当前场地</van-button
      >
    </div>
  </div>
    ...
    .then((res) => {
      ...
      if (res.data.code == 200) {
        ...
        for (let i = 0; i < res.data.data.length; i++) {
          res.data.data[i].coins = Math.floor(
            res.data.data[i].coins / coinFee.value
          ); //取整
          res.data.data[i].coinsBind = Math.floor(
            res.data.data[i].coinsBind / coinFee.value
          );
        }
...
const activeMachine = (item) => {
  ...
  showConfirmDialog({
    title: "温馨提示",
    message:
      '确定要切换<span style="color: rgb(7, 193, 96);">' +
      item.addressName +
      "</span>为当前场地？",
    allowHtml: true,
  })
        ...
        .then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            showDialog({
              title: "温馨提示",
              message:
                '<span style="color: rgb(233, 70, 70);">要立即重新登录切换当前地址才能生效！</span>',
              allowHtml: true,
            }).then(() => {
              location.href = `https://www.huanxizn.com/newbasketball/#/login/?machineId=null&extId=0`;
            });
          } else {
            showToast(res.data.message);
          }
