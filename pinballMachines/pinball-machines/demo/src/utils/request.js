...
axios.defaults.timeout = 3 * 1000 * 60
...
axios.interceptors.request.use(config => {
    if (localStorage.shop_heardTokenStarter) { //判断token是否存在
        config.headers['Authorization'] = 'Bearer ' + localStorage.getItem('shop_heardTokenStarter');  //将token设置成请求头
    }
    return config
}, e => Promise.reject(e))
...
axios.interceptors.response.use(response => {
    ...
    if (error && error.response) {
        switch (error.response.status) {  //处理状态码
            case 400:
                showToast("请先扫设备码，再进入此页面");
                break;
            case 401:
                showToast("未授权，请重新登录");
                // sessionStorage.clear();
                localStorage.removeItem('shop_heardTokenStarter')
                // delCookie('username');
                router.push({
                    path: "/login",
                })
