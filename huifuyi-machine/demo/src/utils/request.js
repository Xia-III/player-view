...
axios.interceptors.request.use(config => {
    console.log(localStorage.washing_heardTokenStarter);
    console.log(localStorage.getItem('washing_heardTokenStarter'));
    config.headers['Authorization'] = 'Bearer ' + localStorage.getItem('washing_heardTokenStarter');  //将token设置成请求头
    ...
    if (error && error.response) {
        switch (error.response.status) {  //处理状态码
            ...
            case 401:
                ...
                localStorage.removeItem('washing_heardTokenStarter')
                // delCookie('username');
                router.push({
                    path: "/login",
                })
