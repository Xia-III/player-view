  ...
  <div>
    <div class="user">
      <div class="topmachine" @click="showchooseM">
        ...
        <p>{{ remark == null || remark == "" ? "普通机器" : remark }}</p>
        <p>
          {{ machineId
          }}<span
            v-if="
    ...
    <div class="main" v-if="isNewPage === false">
      ...
      <div
        ...
        @click="!isButtonDisabled && onInsertCoins(item, playerProfile)"
      >
        <div class="prices">
    ...
    <div class="bigBox" v-if="isNewPage === true">
      ...
      <div class="discounts">
        <div
          class="coinPlan"
          v-for="(item, index) of rechargeConfigLists"
          :key="index"
          @click="onshowShop(item)"
          v-if="rechargeConfigLists.length > 0"
        >
        ...
        <div class="null" v-if="rechargeConfigLists.length <= 0">
          暂无充值套餐
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div class="coinsContent">
        <div
          ...
          v-for="item in coinInPlanList"
          :key="item.coinInPlanId"
          @click="onInsertCoin(item, playerProfile)"
        >
          <div class="coinPlan">
            <div>{{ toyuan(item.fee) }}元</div>
            <div class="yuan">{{ item.coins }}币</div>
...
const onInsertCoins = async (item) => {
  ...
  if (isCoin.value === false) {
      ...
      await v3PlayerCoinInAPI({
        ...
        .then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            showDialog({
              message:
                remark.value + machineId.value + "投" + item.coins + "币成功",
            }).then(() => {
              isButtonDisabled.value = false; // 恢复状态
            });
          } else {
    ...
    } else if (coins.value < item.coins && coinsBind.value < item.coins) {
      //即充即投
      isButtonDisabled.value = false; // 无论成功或失败，启用按钮
...
const toggleImage = () => {
  ...
  if (isCoin.value === false) {
    ...
    console.log("根据币数");
    if (
      coins.value + coinsBind.value >= count.value &&
      coins.value + coinsBind.value > 0
    ) {
      //默认投币
    ...
    } else if (coins.value + coinsBind.value < count.value) {
      //即充即投
      isButton.value = false; // 恢复状态
      console.log(123);
      console.log("余币不足");
  ...
  if (isCoin.value === false) {
    ...
    showConfirmDialog({
      ...
      .then(async () => {
        // on confirm
        // console.log(coins.value);
        if (coins.value + coinsBind.value >= item.coins && coins.value > 0) {
          //默认投币
          showLoading.value = true;
          console.log("余币多");
          console.log(machineId.value);
