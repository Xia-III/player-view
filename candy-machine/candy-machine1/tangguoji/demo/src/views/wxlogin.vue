  ...
  await v3AuthPlayerAuthAPI({
    ...
    .then((res) => {
      ...
      if (res.data.code == 200) {
        ...
        localStorage.setItem("sweets_heardTokenStarter", res.data.data);
        // 设置登录的过期时间24小时即1440分钟 单位分钟
        setStorageExpire("sweets_tokenStarter", res.data.data, 1440);
        const host = window.location.host;
        window.location.href = "https://" + host + "/newsweets/#/toLogin";
      }
...
onMounted(() => {
  ...
  if (ua.match(/MicroMessenger/i) == "micromessenger") {
    if (getStorageExpire("sweets_tokenStarter")) {
      wxGetCode();
    }
  }
