  ...
  await v3PlayerProfileAPI()
    .then((res) => {
      ...
      if (res.data.code == 200) {
        ...
        coinsBind.value = Math.floor(
          res.data.data.coinsBind / res.data.data.coinFee
        );
...
const onInsertCoins = (item) => {
  ...
  showConfirmDialog({
    ...
    .then(async () => {
      ...
      if (
        (coins.value >= item.coins && coins.value > 0) ||
        (coinsBind.value >= item.coins && coinsBind.value > 0)
      ) {
        //默认投币
        showLoading.value = true;
        console.log("余币多");
        ...
        await v3PlayerCoinInAPI({
          ...
          .then((res) => {
            ...
            if (res.data.code == 200) {
              ...
              coinsBind.value = Math.floor(
                res.data.data.coinsBind / coinFee.value
              );
              showDialog({
                message:
                  name.value + machineId.value + "投" + item.coins + "币成功",
              }).then(() => {
                // on close
              });
            } else {
